# -*- coding: utf-8 -*-
"""
Created on Fri Nov 10 22:13:19 2017
...
"""
    On fait la partie sur le "Iris dataset" avec la "LS Newton method"
"""
...
def LSclassification(mu,tol,alpha,beta) :
    """
    ...
    for i in range(101) : # On fait varier tau de 0.1 à 10 par pas de 0.1
        ...
        x_sol,xhist,dgap = barr_method2(Q,p,A,b,x_0,mu,tol,t,alpha,beta)
        w = x_sol[:d]
        c = np.dot(X1,w)
    ...
    plt.title("Taux d'erreurs sur la partie de l'échantillon non utilisée pour l'apprentissage - LS newton")
    plt.xlabel("Tau")
...
alpha = 0.01
beta = 0.5
listau,erreurs,classes = LSclassification(mu,tol,alpha,beta)"""

def genDataSetLS(tau,mu,tol,alpha,beta) :
    """
...
alpha = 0.01
beta = 0.5
Z,classes = genDataSetLS(tau,mu,tol,alpha,beta)"""

def LSDgapVIterationsPrimal(tau,tol,alpha,beta) :
    """
    ...
    plt.title("Duality gap versus cumulative number of Newton steps - Primal - LS newton")
    plt.xlabel("LS newton iterations")
...
alpha = 0.01
beta = 0.5
xsol,xhist = LSDgapVIterationsPrimal(tau,tol,alpha,beta)"""
    
def LSDgapVIterationsDual(tau,tol,alpha,beta) :
    """
    ...
    for i,mu in enumerate(mus) : 
        t = m/eta
        tmp1,tmp2,tmp3 = barr_method2(Q2,p2,A2,b2,x2_0,mu,tol,t,alpha,beta)
        xsol.append(tmp1)
        xhist.append(tmp2)
        dgap.append(tmp3)
        pl.append(plt.semilogy(dgap[i],label='mu={}'.format(mu)))
    
    plt.title("Duality gap versus cumulative number of Newton steps - Dual - LS newton")
    plt.xlabel("LS newton iterations")
    plt.ylabel("Duality gap")
    plt.legend()
    plt.show()
...
alpha = 0.01
beta = 0.5
xsol,xhist = LSDgapVIterationsDual(tau,tol,alpha,beta)"""
